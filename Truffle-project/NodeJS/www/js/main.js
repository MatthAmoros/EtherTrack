//var Warehouse = require('./Wrappers/Warehouse.js');

var etherTrackNS_ABI;
var provider;
var bindedContract = [];

const myABI_PROVIDER_EtherTrackNS = "./ABI/EtherTrackNS.json";
const myABI_PROVIDER_EtherTrackWarehouse = "./ABI/EtherTrackWarehouse.json";
...
function displayWarehouse(name, address)
{
	...
	var unitCode = $("#whUniteCode-" + address.substring(0,10)).val();
		createUnit(address, unitCode, provider);
	});

	$("#whBtnSend-" + address.substring(0,10)).click(function() {
	var unitCode = $("#whUniteCode-" + address.substring(0,10)).val();
	var whAddressTo = $("#whDestAddr-" + address.substring(0,10)).val();
...
function sendUnit(from, to, unit, provider)
{
    //Parsing ABI from JSON file (can be a bit long...)
    $.getJSON(myABI_PROVIDER_EtherTrackWarehouse, function (data) {
        console.log("Parsing done, waiting for events... (Warehouse)");
...
function createUnit(whAddress, unit, provider)
{
    //Parsing ABI from JSON file (can be a bit long...)
    $.getJSON(myABI_PROVIDER_EtherTrackWarehouse, function (data) {
        console.log("Parsing done, waiting for events... (Warehouse)");
